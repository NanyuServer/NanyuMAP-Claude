...
import { MapPin, Route, LogOut, ExternalLink, Map, Settings, Compass, Layers, MessageSquare } from 'lucide-react'

const NAV_ITEMS = [
  { href: '/admin/locations', icon: MapPin, label: '地点管理' },
  { href: '/admin/roads', icon: Route, label: '初中部道路编辑' },
  { href: '/admin/floors', icon: Layers, label: '高中部道路编辑' },
  { href: '/admin/visit-routes', icon: Compass, label: '参观路线' },
  { href: '/admin/feedback', icon: MessageSquare, label: '用户反馈' },
  { href: '/admin/system', icon: Settings, label: '系统操作' },
]
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen" style={{ background: '#080809' }}>
        ...
        <div className="px-5 pt-6 pb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-white/30 text-[10px]">导航管理系统</div>
            </div>
          </div>
        </div>
      ...
      {/* Main */}
      <main className="flex-1 overflow-hidden admin-main">
        {children}
      </main>
    </div>
  )
